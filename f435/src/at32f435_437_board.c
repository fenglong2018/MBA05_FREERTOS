...
//  gpio_pin_mux_config(PRINT_UART_TX_GPIO, PRINT_UART_RX_PIN_SOURCE, PRINT_UART_TX_PIN_MUX_NUM);
//  
//  /* 中断优先级配置 */
...
//{
//  uint8_t u8RxData;
//  if (usart_interrupt_flag_get(PRINT_UART, USART_ROERR_FLAG) == SET)
//  {   
//    usart_flag_clear(PRINT_UART,USART_ROERR_FLAG);
//  }
//  if(usart_interrupt_flag_get(PRINT_UART, USART_RDBF_FLAG) != RESET)
//  {
//    usart_flag_clear(PRINT_UART,USART_RDBF_FLAG);
//    u8RxData = usart_data_receive(PRINT_UART);
...
// {
//   usart_data_transmit(PRINT_UART, (uint16_t)ch);
//   while(RESET == usart_flag_get(PRINT_UART, USART_TDC_FLAG));
//   return ch;
// }
